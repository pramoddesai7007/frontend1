...
const UnitList = () => {
  ...
  useEffect(() => {
    const fetchUnits = async () => {
      try {
        const response = await axios.get('https://backend1-ashen.vercel.app//api/unit/units');
  ...
  const handleEditSubmit = async (e, updatedUnit) => {
    ...
    try {
      ...
      await axios.patch(`https://backend1-ashen.vercel.app//api/unit/units/${selectedUnit._id}`, {
  ...
  const handleDeleteSubmit = async () => {
    try {
      await axios.delete(`https://backend1-ashen.vercel.app//api/unit/units/${selectedUnit._id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post('https://backend1-ashen.vercel.app//api/unit/units', { unit: newUnit });
