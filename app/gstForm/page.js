...
const GSTForm = () => {
  ...
  const handleConfirmDelete = async () => {
    try {
      ...
      await axios.delete(`https://backend1-ashen.vercel.app//api/gst/gst/${deletingItemId}`);
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      await axios.post('https://backend1-ashen.vercel.app//api/gst/create', { gstPercentage });
      fetchGSTList();
      setGSTPercentage('');
    } catch (error) {
