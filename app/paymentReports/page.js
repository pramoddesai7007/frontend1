...
const PaymentList = () => {
    ...
    useEffect(() => {
        const fetchPayments = async () => {
            try {
                setLoading(true);
                const response = await axios.get('https://backend1-ashen.vercel.app//api/payment/paymentsList');
                const paymentsWithTableNames = await Promise.all(
                    response.data.map(async (payment) => {
                        const tableResponse = await axios.get(`https://backend1-ashen.vercel.app//api/table/tables/${payment.tableId}`);
                        return {
                            ...payment,
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody>
                            ...
                            <tr className="bg-gray-100">
                                <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200" colSpan="5"></td>
                                <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200 font-bold text-center">
                                    Total Amount:
                                </td>
