...
const capitalizeFirstLetter = (str) => {
    ...
    if (name === "unit") {
      ...
      try {
        const unitDetailsResponse = await axios.get(
          `https://backend1-ashen.vercel.app//api/unit/units/${value}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (itemToEdit) {
        ...
        await axios.put(
          `https://backend1-ashen.vercel.app//api/item/items/${itemToEdit._id}`,
          formData
        );
      } else {
        // If creating, make a POST request
        await axios.post("https://backend1-ashen.vercel.app//api/item/items", {
          ...formData,
  ...
  const handleDeleteConfirmed = async () => {
    try {
      ...
      await axios.delete(
        `https://backend1-ashen.vercel.app//api/item/items/${itemIdToDelete}`
      );
  ...
  //     await axios.delete(`https://backend1-ashen.vercel.app//api/item/items/${itemId}`);
