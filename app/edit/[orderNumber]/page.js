...
const EditOrderPage = ({ params, tableId }) => {
      ...
      try {
        if (orderNumber) {
          ...
          const orderResponse = await axios.get(
            `https://backend1-ashen.vercel.app//api/order/get/order/${orderNumber}`
          );
          ...
          const sectionResponse = await axios.get(
            `https://backend1-ashen.vercel.app//api/section/sectionlist/${tableId}`
          );
  ...
  const handleSaveBill = async () => {
    try {
      ...
      await axios.post(`https://backend1-ashen.vercel.app//api/logHistory/log-history`, {
      ...
      const updateOrderEndpoint = `https://backend1-ashen.vercel.app//api/order/update-order-by-number/${orderNumber}`;
  ...
  const handlePrintBill = async () => {
    try {
      ...
      const tableResponse = await axios.get(
        `https://backend1-ashen.vercel.app//api/table/tables/${tableId}`
      );
