...
const Section = () => {
  ...
  useEffect(() => {
    const fetchSections = async () => {
      try {
        const response = await axios.get("https://backend1-ashen.vercel.app//api/section");
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      await axios.patch(
        `https://backend1-ashen.vercel.app//api/section/${sectionToEdit._id}`,
        {
  ...
  const handleDeleteConfirmed = async () => {
    ...
    try {
      await axios.delete(
        `https://backend1-ashen.vercel.app//api/section/${sectionToDelete._id}`
      );
      ...
      const sectionsResponse = await axios.get(
        "https://backend1-ashen.vercel.app//api/section"
      );
  ...
  const handleAddSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        "https://backend1-ashen.vercel.app//api/section/create",
        formData,
        {
          headers: {
