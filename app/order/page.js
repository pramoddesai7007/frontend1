...
const Billing = ({ tableId, acPercentage }) => {
  ...
  useEffect(() => {
    const fetchTastes = async () => {
      try {
        const response = await axios.get('https://backend1-ashen.vercel.app//api/taste/tastes');
...
`).join("");
  ...
  const fetchWaitersList = async () => {
    try {
      const response = await axios.get('https://backend1-ashen.vercel.app//api/waiter');
    ...
    const fetchGreetings = async () => {
      try {
        const response = await axios.get(
          "https://backend1-ashen.vercel.app//api/greet/greet"
        );
  ...
  useEffect(() => {
    ...
    const fetchNextOrderNumber = async () => {
      try {
        const response = await axios.get(
          'https://backend1-ashen.vercel.app//api/order/get-next-order-number'
        );
  ...
  const fetchLastAllOrders = async () => {
    try {
      const ordersResponse = await axios.get(
        "https://backend1-ashen.vercel.app//api/order/latest-orders"
      );
      ...
      const tableNamesPromises = orders.map(async (order) => {
        const tableResponse = await axios.get(
          `https://backend1-ashen.vercel.app//api/table/tables/${order.tableId}`
        );
  ...
  const saveBill = async () => {
    try {
      ...
      const existingBillResponse = await axios.get(
        `https://backend1-ashen.vercel.app//api/order/order/${tableId}`
      );
      ...
      if (existingBill && existingBill.length > 0) {
        ...
        const updateResponse = await axios.patch(
          `https://backend1-ashen.vercel.app//api/order/update-order-by-id/${orderIdToUpdate}`,
          orderData
        );
      ...
      const KOTResponse = await axios.post(
        `https://backend1-ashen.vercel.app//api/kot/kotOrder/${tableId}`,
        kotData);
  ...
  const saveKot = async () => {
    try {
      ...
      const existingKOTResponse = await axios.get(
        `https://backend1-ashen.vercel.app//api/kot/kot/${tableId}`
      );
...
<p style="${!hotelInfo || !hotelInfo.sacNo ? "display: none;" : ""}">SAC No: ${hotelInfo ? hotelInfo.sacNo : "SAC No Not Found"
  ...
  useEffect(() => {
    ...
    // Fetch categories
    axios
      .get("https://backend1-ashen.vercel.app//api/main")
      .then((response) => {
    ...
    // Fetch products
    axios
      .get("https://backend1-ashen.vercel.app//api/menu/menus/list")
      .then((response) => {
        console.log(response.data);
    ...
    if (tableId) {
      axios
        .get(`https://backend1-ashen.vercel.app//api/table/tables/${tableId}`)
        .then((response) => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory) {
      axios
        .get(`https://backend1-ashen.vercel.app//api/menu/${selectedCategory._id}`)
        .then((response) => {
          console.log(response.data);
          const menusArray = response.data || []; // Ensure menus is an array
          setMenus(menusArray);
  ...
  const handleCategoryClick = (category) => {
    ...
    if (category === null) {
      ...
      // Fetch menus based on the selected category
      axios
        .get(`https://backend1-ashen.vercel.app//api/menu/menulist/${category._id}`)
        .then((response) => {
          setMenus(response.data);
        })
        .catch((error) => {
    ...
    const fetchHotelInfo = async () => {
      try {
        ...
        const allHotelsResponse = await axios.get(
          "https://backend1-ashen.vercel.app//api/hotel/get-all"
        );
        ...
        if (defaultHotelId) {
          ...
          const response = await axios.get(
            `https://backend1-ashen.vercel.app//api/hotel/get/${defaultHotelId}`
          );
                ...
                >
                  <div className="flex flex-row items-center w-3/5">
                    <div className="flex items-center h-full ">
                      <img
                        // src={`https://backend1-ashen.vercel.app//${orderItem.imageUrl}`}
                        src={`/tray.png`}
